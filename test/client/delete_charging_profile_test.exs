defmodule PorscheConnEx.ClientDeleteChargingProfileTest do
  ...
  describe "delete_charging_profile/4" do
    test "issues and completes pending request", %{session: session, bypass: bypass} do
      ...
      profile_id = Enum.random(5..7)

      base_url = "/e-mobility/de/de_DE/#{model}/#{vin}"

      Bypass.expect_once(bypass, "DELETE", "#{base_url}/profile/#{profile_id}", fn conn ->
        resp_json(conn, ServerResponses.action_in_progress(req_id))
      end)

      # Issue initial request:
      assert {:ok, pending} = Client.delete_charging_profile(session, vin, model, profile_id)
